...
def tidigit2labels(tidigitsarray):
    ...
    for ex in range(nex):
        labels.append(
            tidigitsarray[ex]["gender"]
            + "_"
            + tidigitsarray[ex]["speaker"]
            + "_"
...
def trfbank(
    ...
    for i in range(nfilt):
        ...
        lid = np.arange(
            np.floor(low * nfft / fs) + 1, np.floor(cen * nfft / fs) + 1, dtype=int
        )
        lslope = heights[i] / (cen - low)
        rid = np.arange(
            np.floor(cen * nfft / fs) + 1, np.floor(hi * nfft / fs) + 1, dtype=int
        )
